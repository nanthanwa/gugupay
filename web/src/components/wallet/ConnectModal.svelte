<script lang="ts" module>
  ...
  let _resolve =
    $state<
      (value: IWallet | PromiseLike<IWallet | undefined> | undefined) => void
    >();
  let resolve = {
    get value() {
      return _resolve;
...
<script lang="ts">
  ...
  let { availableWallets }: IProps = $props();
    ...
    <div class="grid gap-y-4">
      <div class="grid grid-cols-2">
        ...
        <div class="flex justify-end">
          <button
            class="btn-icon variant-filled-error btn-sm"
            aria-label="Close"
            onclick={onClose}
          >
      ...
      {:else}
        <div class="flex flex-col">
          {#each availableWallets as wallet (wallet.name)}
            <button
              class="font btn btn-lg bg-base-100 flex items-center justify-start space-x-4 border-none shadow-none"
